...
def evaluate_regression_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy: float=0.7) -> MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            # Calculating harmonic mean of train_accuracy and test_accuracy for r2_score metric
            model_accuracy = (2 * (train_acc * test_acc)) / (train_acc + test_acc)
            diff_test_train_acc = abs(test_acc - train_acc)

            # Logging all important metrics calculated
            logging.info(f"{'>>'*20} r2_score {'<<'*20}")
            ...
            # then we will accept that model as accepted model;
...
class ModelFactory:
    ...
    def update_property_of_class(instance_ref:object, property_data: dict):
        try:
            if not isinstance(property_data, dict):
                raise Exception("property_data parameter required for dictionary")
            print(property_data)
    ...
    def class_for_name(module_name:str, class_name:str):
        try:
            ...
            # get the class, will raise AttributeError is class cannot be found
    ...
    def execute_grid_search_operation(self, initialized_model: InitializedModelDetail, input_feature,
        ...
        """
        execute_grid_search_operation(): function will perform parameter search operation and it will return you
